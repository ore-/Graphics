...
namespace AIGraphics.Setting
{
    internal class TextureSetting
    {
        ...
        public int index = 0;
        public Texture2D Texture => index > 0 ? Textures.ToArray()[index] : null;
        public string TexturePath => index > 0 ? TexturePaths.ToArray()[index] : null;
        ...
        {
            return TexturePaths.FindIndex(x => x.Equals(path));
        }

        public void SetIndexByPath(string path)
        {
            index = FindIndexByPath(path);
        }
